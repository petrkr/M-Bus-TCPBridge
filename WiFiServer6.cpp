...
int WiFiServer6::setTimeout(uint32_t seconds){
  struct timeval tv;
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
...
size_t WiFiServer6::write(const uint8_t *data, size_t len){
  return 0;
}

void WiFiServer6::stopAll(){}

WiFiClient WiFiServer6::available(){
  if(!_listening)
    return WiFiClient();
  int client_sock;
...
void WiFiServer6::begin(uint16_t port){
    begin(port, 1);
}

void WiFiServer6::begin(uint16_t port, int enable){
  if(_listening)
    return;
  if(port){
      _port = port;
...
void WiFiServer6::setNoDelay(bool nodelay) {
    _noDelay = nodelay;
}

bool WiFiServer6::getNoDelay() {
    return _noDelay;
}

bool WiFiServer6::hasClient() {
    if (_accepted_sockfd >= 0) {
      return true;
    }
    struct sockaddr_in _client;
...
void WiFiServer6::close(){
  end();
}

void WiFiServer6::stop(){
  end();
}
